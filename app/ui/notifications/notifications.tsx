  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        if (response.ok) {
          // You can use setNotifications here if needed
        } else {
      ...
      {notifications.length > 0 ? (
        <div className="space-y-4 max-h-60 overflow-y-auto">
          {notifications.map((notification) => (
            <Link
              key={notification.id}
              href={`/dashboard/notification-details/${notification.id}`}
        ...
        !loading && <p className="text-gray-500 italic">No notifications yet.</p>
      )}
    </div>
  );
}
